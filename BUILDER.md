...
Rams × NYCTA is a module: the two-master thesis, the NYCTA subsystem, and the
Nonism philosophy. The system law it sits inside is
[Axiom Design Core](https://github.com/Nonarkara/Axiom-Design-Core).

Where the method bites hardest here is §2. Wayfinding is the purest case of
"imagine the human first" in the whole estate — a sign works or fails in the
three seconds a person has while walking, carrying something, in a hurry, in a
language that may not be theirs. Vignelli's diagram is what happens when someone
models that person precisely and then removes everything the model does not
need.

You cannot get there by styling. Walk the route.
